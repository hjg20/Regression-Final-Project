...
After finding the $R^2$ score of each regression, we see that WLS has a better score. Therefore, we will choose the WLS model as the better model because it has a higher percentage of the variance of price being explained by the model.
...
$$\text{Price}_i = 2.14 + 0.00000580 \, \text{area}_i + 0.00290 \, \text{bedrooms}_i + 0.0202 \, \text{bathrooms}_i + 0.0103 \, \text{stories}_i + 0.0129 \, \text{mainroad}_i\text{yes}$$ $$+0.00822 \, \text{guestroom}_i\text{yes} + 0.00752 \, \text{basement}_i\text{yes} + 0.0189 \, \text{hotwaterheating}_i\text{yes} + 0.0206 \, \text{airconditioning}_i\text{yes}$$ $$+0.00469 \, \text{parking}_i + 0.0157 \, \text{prefarea}_i\text{yes} + 0.00134 \, \text{furnishingstatus}_i\text{semi-furnished} - 0.0131 \, \text{furnishingstatus}_i\text{unfurnished}$$
...
There were some issues with the data set. The first is that we were expecting collinearity in variables number of bedrooms and number of bathrooms. However there was no collinearity between these variables. This points to the fact that the data may be flawed in some way. Perhaps the dataset is artificially created or random since in real life the number of bedrooms and bathrooms should be correlated in some way. Also, we would add more columns to this data set such as location, proximity to major city, and the average income of each location. These variables are important because a house near the city will cost much more than a house in a rural area. It would be easier to predict housing prices given the location of the house and the proximity to a major city.
